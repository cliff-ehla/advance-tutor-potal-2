...
<a href="/students/{student_id}/tutor-group/{course.tutor_group_id}" class="block p-4 border hover:border-blue-500 hover:shadow-lg transition-shadow rounded {in_progress ? 'bg-white border-blue-300' : 'bg-blue-50 border-gray-200'} border border-gray-200">
	...
	<div class="lg:flex items-center">
		...
		{:else}
			<div class="text-xs bg-yellow-500 rounded px-2 text-white">
				<p>{course.completed_zoom_cnt} of {total_zoom_cnt}</p>
			</div>
		{/if}
	</div>
</a>