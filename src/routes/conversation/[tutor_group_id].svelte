<script>
	...
	$: course = $conversation_store ? conversation_store.getConversation(tutor_group_id) : undefined
	$: student_id = course && course.student_id
</script>
	...
	<a href="/students/{student_id}" class="group h-20 flex items-center border-b border-gray-200 px-4">
		<div class="w-full flex items-center">
			<div class="w-12 h-12 rounded-full border border-gray-400 relative shadow flex-shrink-0">
				<img src="/student-{course.gender}-icon.png" alt="gender" class="rounded-full">
				<div class="absolute shadow font-bold border border-white -bottom-2 -right-2 ml-2 w-7 h-7 bg-blue-500 rounded-full text-xs cc text-white">{capitalize(course.level)}</div>
			</div>
			<div class="ml-4">
				<p style="font-size: 1.8em" class="group-hover:text-blue-500 mt-1 font-light text-xl text-gray-700 leading-none">{course.nickname}</p>
			</div>
...
{:else}
	<div class="p-8">
		...
		<div class="note">
			The selected tutor group (ID:{tutor_group_id}) is not returned from /list_zoom_status_in_tutor_group
		</div>
	</div>
{/if}
{#if tutor_group_id}
